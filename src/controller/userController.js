...
const login = async (req, res) => {
  try {
    ...
    if (user.userLogs.length >= 2) {
      ...
      if (a && b && a.logoutTime === "") {
        a.logoutTime = `${b.loginTime} - ${now.toLocaleDateString("en-US", {
          weekday: "long",
          year: "numeric",
          month: "long",
          day: "numeric",
...
const logout = async (req, res) => {
  try {
    ...
    latestLog.logoutTime = `${logoutTime} -${now.toLocaleDateString("en-US", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric",
...
const get_all_user_staffs = async (req, res) => {
  try {
    const user_staff = await userModel
      .find({ role: { $ne: "Admin" } })
      .sort({ createdAt: -1 })
      .lean();
    if (!user_staff) {
...
const export_staffs = async (req, res) => {
  try {
...
const update_details = async (req, res) => {
  try {
    ...
    const { name, email, phone_no, gender, role } = req.body;
    // check for userId
    if (!userId) {
      return res.status(400).json({
        success: false,
...
const staff_forget_pass_otp = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "OTP sent to email", data: staff.email });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "Server error",
...
const staff_verify_otp = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "OTP verified", userId: userOTP.userId });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "Server error",
...
const active_inactive_staff_user = async (req, res) => {
  try {
    ...
    const staff_user = await userModel.findOne({
      _id: staff_user_id,
      role: { $ne: "Admin" },
    });
    if (!staff_user) {
    ...
    return res.status(200).json({
      ...
      Hospital_Details: all_hospital.map((h) => {
        const uniquePatientCount = new Set(
          (h.PatientAssigned || []).map((p) => p.patientId?.toString())
        ).size;
...
const getPatientsByHospitalId = async (req, res) => {
  ...
  try {
    ...
    const patientIds = (hospital.PatientAssigned || [])
      .map((p) => p.patientId?.trim())
      .filter(Boolean);
    ...
    if (uniquePatientIds.length === 0) {
      return res.status(200).json({
        success: true,
        message: "No patients assigned to this hospital",
        patients: [],
      });
    }
      ...
      if (relationFile) {
        ...
        if (imageFields.includes(ext)) {
          ...
          return res.status(400).json({
            success: false,
            message:
              "Invalid file type for relation_id. Only .jpg, .jpeg, and .png allowed.",
          });
        }
      }
      ...
      if (idProofFile) {
        ...
        if (imageFields.includes(ext)) {
          ...
          return res.status(400).json({
            success: false,
            message:
              "Invalid file type for patient_id_proof. Only .jpg, .jpeg, and .png allowed.",
          });
        }
      }
    }
...
const update_Enquiry_status = async (req, res) => {
  try {
    ...
    if (enquiry.enq_status === "Confirmed") {
      const exist_patient = await patientModel.findOne({
        email: enquiry.email,
      });
      ...
      if (exist_patient) {
        ...
        exist_patient.town = enquiry.town;
        ...
        const patientId = `OMCA${generateRandomNumber(5)}`;
    ...
    if (enquiry.emergency_contact_no && enquiry.phoneCode) {
      const phoneCode = enquiry.phoneCode.startsWith("+")
        ? enquiry.phoneCode
        : `+${enquiry.phoneCode}`;
...
const export_enquiries = async (req, res) => {
  try {
    ...
    if (!enquiries.length) {
      return res.status(404).json({
        success: false,
        message: "No pending enquiries found to export",
      });
    }

    const workbook = new ExcelJs.Workbook();
    const worksheet = workbook.addWorksheet("Pending Enquiries");
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=pending_enquiries.xlsx"
    );
...
const update_notes = async (req, res) => {
  try {
    ...
    // Update note in patient data
    const patientNote = patientData.discussionNotes.id(id);
    if (patientNote) {
      if (note) patientNote.note = note;
      if (date) patientNote.date = date;
      await patientData.save();
    }

    // Get enquiryId from patient data and update corresponding enquiry
    const enquiryId = patientData.enquiryId;
    if (enquiryId) {
      const enquiryData = await enquiryModel.findOne({ enquiryId });
      if (enquiryData) {
        const enquiryNote = enquiryData.discussionNotes.id(id);
        if (enquiryNote) {
          if (note) enquiryNote.note = note;
          if (date) enquiryNote.date = date;
          await enquiryData.save();
        }
      }
    }
...
const export_patients = async (req, res) => {
  try {
    const patients = await patientModel.find({}).lean(); // Add filters if needed
    ...
    patients.forEach((patient) => {
      worksheet.addRow({
        ...
        email: patient.email,
        country: patient.country,
        disease_name: patient.patient_disease[0].disease_name,
      });
    });
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=patients_export.xlsx"
    );
  ...
  try {
    ...
    const isValidHeaders = requiredHeaders.every((header, index) => {
      return (
        ...
        actualHeaders[index]?.toLowerCase().trim()
      );
    });
          ...
          email:
            ...
            row.getCell(3).value !== null
              ? row.getCell(3).value.text ||
                row.getCell(3).value.hyperlink.split("mailto:")[1]
              : row.getCell(3).value,
...
const update_patient = async( req , res )=> {
                ...
                if(!patientId)
                {
                   ...
                   if(!patient)
                   {
                    return res.status(400).json({
                       ...
                       if(patientDisease)
                        {
                           patient.patient_disease[0].disease_name = patientDisease
                        }                      
 ...
 if (date) {
  ...
  if (!isNaN(parsedDate.getTime())) {
    ...
    // 🔥 Disable auto timestamp just for this save
    patient.$__.saveOptions = { timestamps: false };
  } else {
...
const changePatientStatus = async (req, res) => {
  try {
    ...
    if (!p_status) {
      return res.status(400).json({
        success: false,
        message: "p_status is required",
      });
    }

    // Validate p_status value
    const allowedStatuses = [
      "Foundation", "Private", "Travelled", "Confirmed", "Pending",
      "On Hold", "Cancelled", "Local Case", "Follow Up", "Passed Away"
    ];

    if (!allowedStatuses.includes(p_status)) {
      return res.status(400).json({
        success: false,
        message: "Invalid p_status value",
      });
    }
...
const create_appointment = async (req, res) => {
  try {
    ...
    if (patient.emergency_contact_no && patient.phoneCode) {
      const phoneCode = patient.phoneCode.startsWith("+")
        ? patient.phoneCode
        : `+${patient.phoneCode}`;
      const fullNumber = `${phoneCode}${String(patient.emergency_contact_no)}`;

      const waMessage = `Hello ${patient.patient_name}, your appointment for "${
...
const export_appointments = async (req, res) => {
  try {
    ...
    worksheet.columns = [
      ...
      { header: "Treatment Name", key: "treatment_name", width: 25 },
      { header: "Appointment ID", key: "appointmentId", width: 25 },
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=appointments_export.xlsx"
    );
...
const get_treatment_course_by_id = async (req, res) => {
  try {
    ...
    const treatment_course = await treatement_course_model.findById(
      treatment_course_id
    );
    if (!treatment_course) {
      return res.status(400).json({
...
const create_treatment = async (req, res) => {
  try {
    ...
    const treatment_course = await treatement_course_model.findOne({
      _id: treatment_course_id,
    });
    if (!treatment_course) {
      return res
        .status(400)
        .json({ success: false, message: "Treatment Course Not Found" });
    }
    ...
    if (patient.emergency_contact_no) {
      try {
        const whatsappResult = await sendWhatsAppMessage(
          `${patient.phoneCode}${patient.emergency_contact_no}`,
          waMessage
        );
  ...
  try {
    ...
    if (!["InProgress", "Complete", "Cancelled"].includes(status)) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid status. You can only update to InProgress, Complete, or Cancelled.",
      });
    }
...
const Dashboard_count = async (req, res) => {
  try {
    ...
    const all_Enquiry = await enquiryModel.countDocuments({
      enq_status: { $ne: "Confirmed" },
    });
    ...
    const totalTreatmentCourses =
      await treatement_course_model.countDocuments();

    // Get all treatment courses
    const allCourses = await treatement_course_model.find(
      {},
      "course_name image"
    );
    ...
    for (let course of allCourses) {
      ...
      const count = await patientModel.countDocuments({
        treatment_course_name: courseName,
      });

      courseAssignmentCounts.push({
...
const add_new_treatment_payment = async (req, res) => {
  try {
    ...
    if (patient && patient.emergency_contact_no && patient.phoneCode) {
      ...
      }, we have received your payment of ₹${paid_amount} for treatment "${
        treatment.treatment_course_name
      }".\n\n💳 Payment Method: ${paymentMethod}\nDate: ${new Date(
        payment_Date
      ).toLocaleDateString()}\nRemaining Balance: ₹${
        treatment.duePayment
    ...
    const earnings = await Promise.all(
      treatments.map(async (treatment) => {
        ...
        return {
          ...
          total_Amount: treatment.totalCharge,
          amount_paid: treatment.totalCharge - treatment.duePayment,
          remaining_balance: treatment.duePayment,
        };
      })
    );
...
const get_all_treatment_courses = async (req, res) => {
  try {
    const courses = await treatement_course_model
      .find({})
      .sort({ createdAt: -1 })
      .lean();
    ...
    for (const course of courses) {
      const treatments = await treatmentModel
        .find({ treatment_course_id: course._id })
        .lean();
      ...
      if (patientIds.length > 0) {
        const patients = await patientModel
          .find({ patientId: { $in: patientIds } }, { country: 1, _id: 0 })
          .lean();
        ...
        const sortedCountries = Object.entries(countryCountMap).sort(
          (a, b) => b[1] - a[1]
        );
...
const getReports = async (req, res) => {
  try {
    ...
    const getAllReports = await reportModel.find({ patientId });
...
module.exports = {
  ...
  staff_forget_pass_otp,
  staff_verify_otp,
  staff_reset_password,
  active_inactive_staff_user,
  logout,
  refreshToken,
  ...
  get_treatment_course_by_id,
  update_treatment_course,
  delete_treatment_course,
  ...
  update_patient_treatment_status,
  add_new_treatment_payment,
  update_treatment_status,
  get_unadded_services_for_treatment,
